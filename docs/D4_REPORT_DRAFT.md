...
| ROS4HRI / ROS4RI | **Used.** The adapter publishes `hri_actions_msgs/Intent` for the **Odoo planner manufacturing-order intent**; companion nodes (in the upstream `hermes_main/hololens_api` package) publish `Intent` for the HoloLens AR operator actions (project selection / placement confirmation / assembly complete). Mapping reuses standard constants where they fit and domain labels otherwise — see [`D4_PLAN.md`](D4_PLAN.md) §4.4 for the full table. The message envelope is unchanged (no extension). | `D4_PLAN.md` §4.4. |
