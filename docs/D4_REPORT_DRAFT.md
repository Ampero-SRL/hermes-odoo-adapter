...
| Question | Answer |
|---|---|
| How does the module use Vulcanexus? | Built on the official `eprosima/vulcanexus:humble` Docker image, **`vulcanexus-humble-core 2.8.0`** (with `vulcanexus-humble-fastdds 2.14.4` + `vulcanexus-humble-fastcdr 1.1.1`). Uses `rclpy` for the ROS 2 node, the `rmw_fastrtps_cpp` RMW shim (`ros-humble-rmw-fastrtps-cpp 6.2.7`) over `ros-humble-fastrtps 2.6.10`, and the ROS 2 message generation tooling (`rosidl_default_generators`) to colcon-build the vendored `hermes_msgs` + the upstream `hri_actions_msgs` from `ros2_ws/`. |
| Which Vulcanexus / Fast-DDS-specific features are used or validated? | **In use today:** single-process `rclpy` ↔ FastAPI co-existence inside one container, discovery on `ROS_DOMAIN_ID=42`, **latched durability** (`TRANSIENT_LOCAL`+`KEEP_LAST/1`) for the `/hermes/warehouse/tray_state` topic, default `RELIABLE`+`KEEP_LAST/10` QoS for everything else, and the standard `rmw_fastrtps_cpp` transport over UDP. **Available in the base image but not yet exercised** (Sprint-1.5-relevant honesty): the Vulcanexus Discovery Server / Easy Mode for cross-network deployments, the DDS Router / DDS Recorder for trace replay, DDS Keys / Dynamic Types for schema evolution, and custom XML QoS profiles. These are listed as future work in §3.3.10. |
