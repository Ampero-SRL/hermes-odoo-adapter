...
| **ROS4HRI / ROS4RI** | The adapter publishes `hri_actions_msgs/Intent` for the **Odoo planner manufacturing-order intent** (`intent=START_ACTIVITY`, `source=REMOTE_SUPERVISOR`, `modality=MODALITY_OTHER`). Operator-side intents from the HoloLens AR app (project selection, placement confirmation, assembly complete) are published from companion ROS 2 nodes closer to the source — see [`02_interfaces.md`](02_interfaces.md) for the full mapping. |
